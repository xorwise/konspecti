...
\begin{enumerate}
  ...
  \item $(\int(f(x) \pm g(x))dx)' = f(x) \pm g(x)$
    \\ $(\int (fx)dx \pm \int g(x)dx)' = (\int f(x)dx)' \pm (\int g(x)dx)' = f(x) \pm g(x)$
\end{enumerate}
...
\begin{enumerate}
  ...
  \item $\int \frac{1}{\sin(x)^2}dx = -\cot(x) + C$ 
 ...
 \item $\int \frac{1}{1 + x^2}dx =$
   ...
   \end{cases}
      \item $\int \frac{1}{\sqrt{k^2 - x^2} }dx =$
        \begin{cases}
         $\arcsin \left \frac{x}{k} \right + C$ \\
         $-\arccos \left \frac{x}{k} \right + C$
        \end{cases}
